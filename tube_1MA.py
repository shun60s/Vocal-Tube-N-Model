...
#  simulated opening mouth from (almost) closed mouth /a/ to /a/
...
class Class_CMA2MA(object):
	def __init__(self, target=None, start=None, transient_time=50, start_hold_time=13,  sampling_rate=48000, tc=0.1):
		...
		if start is None:
			# start (almost) closed mouth /a/
			L1_a_s=9.0    # set list of 1st tube's length by unit is [cm]
			A1_a_s=0.1   # set list of 1st tube's area by unit is [cm^2]
			L2_a_s=8.0    # set list of 2nd tube's length by unit is [cm]
			A2_a_s= self.get_A2( A1_a_s, 0.75) 
	...
	def f_show_all_2(self, c1A, label):
		# show comparison with other class c1A
		# show x curve
		fig = plt.figure()
		...
		plt.xlabel('mSec')
		#plt.ylabel('level')
		plt.plot( np.arange(len(self.A1)) / (self.sampling_rate / 1000.0)  , self.A1, color='r')
		plt.plot( np.arange(len(c1A.A1)) / (c1A.sampling_rate / 1000.0)  , c1A.A1, color='b')
		
		plt.subplot(4,1,2)
		plt.title( 'A2' + label)
		plt.xlabel('mSec')
		#plt.ylabel('level')
		plt.plot( np.arange(len(self.A2)) / (self.sampling_rate / 1000.0)  , self.A2, color='r')
		plt.plot( np.arange(len(c1A.A2)) / (c1A.sampling_rate / 1000.0)  , c1A.A2, color='b')
		
		plt.subplot(4,1,3)
		plt.title( 'L1' + label)
		plt.xlabel('mSec')
		#plt.ylabel('level')
		plt.plot( np.arange(len(self.L1)) / (self.sampling_rate / 1000.0)  , self.L1, color='r')
		plt.plot( np.arange(len(c1A.L1)) / (c1A.sampling_rate / 1000.0)  , c1A.L1, color='b')
		
		plt.subplot(4,1,4)
		plt.title( 'L2' + label)
		plt.xlabel('mSec')
		#plt.ylabel('level')
		plt.plot( np.arange(len(self.L2)) / (self.sampling_rate / 1000.0)  , self.L2, color='r')
		plt.plot( np.arange(len(c1A.L2)) / (c1A.sampling_rate / 1000.0)  , c1A.L2, color='b')
...
if __name__ == '__main__':
	...
	from tube_1NA import *
	B=Class_neutral2NA()
	A.f_show_all_2(B, '(red: /ma/  green: /na/)' )
